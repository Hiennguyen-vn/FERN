...
import { AsyncJobProgress, Button, Card, EmptyState, ErrorState, ReadonlyBanner } from '@design-system/index'
    ...
    <DashboardLayout description="Poll and inspect a single export job." title={`Export Job ${jobId ?? ''}`}>
      ...
      {job ? (
        <>
          ...
          <AsyncJobProgress
            actionLabel={canPreviewExport(job) ? 'Open preview' : undefined}
            completedAt={job.completedAt}
            description={getExportStatusDescription(job)}
            onAction={canPreviewExport(job) ? () => (window.location.href = `/reports/export-jobs/${job.exportJobId}/preview`) : undefined}
