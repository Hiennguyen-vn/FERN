...
export function PaymentReviewModule() {
  ...
  const handleApprove = (pay: SupplierPayment) => {
    setPayments(prev => prev.map(p => p.id === pay.id ? { ...p, status: 'approved' as any, reviewedBy: 'Current User', reviewedAt: new Date().toISOString() } : p));
    setSelected(null);
    toast.success('Payment approved');
  };

  const handleProcess = (pay: SupplierPayment) => {
    setPayments(prev => prev.map(p => p.id === pay.id ? { ...p, status: 'processed' as any, processedAt: new Date().toISOString() } : p));
    setSelected(null);
  ...
  const handleReject = () => {
    ...
    setPayments(prev => prev.map(p => p.id === selected.id ? { ...p, status: 'rejected' as any, rejectionReason: rejectReason, reviewedBy: 'Current User', reviewedAt: new Date().toISOString() } : p));
    setRejectOpen(false);
    setRejectReason('');
    setSelected(null);
