...
export function BranchBlockedBanner() {
  return (
    <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-warning/8 border border-warning/15">
      ...
      <div>
        <p className="text-xs font-semibold text-foreground">IAM Service — Branch Blocked</p>
        <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">
          iam-service fails startup due to Flyway migration <code className="text-[10px] bg-muted px-1 rounded">V19__pos_customer_permissions.sql</code>. 
          These screens show the target-state administration design. All write operations are unavailable until the migration is resolved.
        </p>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="surface-elevated p-4 space-y-2.5">
            {[
              { label: 'Auth Service', status: 'blocked', detail: 'V19 migration failure' },
              { label: 'User Store', status: 'available', detail: 'Read-only via gateway' },
              { label: 'Permission Engine', status: 'blocked', detail: 'Depends on iam-service' },
              { label: 'Session Manager', status: 'degraded', detail: 'Fallback auth active' },
            ].map(s => (
