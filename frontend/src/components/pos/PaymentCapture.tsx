...
export function PaymentCapture({ orderTotal, lineItems, promoCode, promoDiscount, subtotal, taxAmount, onBack, onComplete }: Props) {
  ...
  const handleConfirm = () => {
    ...
    setTimeout(() => {
      setProcessing(false);
      setSuccess(true);
      // Use the first split's method as primary
      onComplete(splits[0]?.method || 'cash');
    }, 1000);
  };
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8 animate-fade-in">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-xl font-semibold text-foreground mb-1">Order Completed</h2>
          <p className="text-sm text-muted-foreground mb-2">Payment of ${orderTotal.toFixed(2)} captured successfully</p>
          <div className="mt-4 p-3 rounded-lg bg-muted/40 text-left space-y-1">
            {splits.map((s, i) => (
              <div key={i} className="flex justify-between text-xs">
                <span className="text-muted-foreground capitalize">{s.method.replace('-', ' ')}</span>
                <span className="text-foreground font-medium">${s.amount.toFixed(2)}</span>
              </div>
            ))}
          </div>
          <Button className="mt-6 h-9 text-xs" onClick={onBack}>
            Back to Session
          </Button>
        </div>
      </div>
          ...
          {failed && (
            <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-destructive/5 border border-destructive/15">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">Transaction was declined. Please try again or use a different method.</p>
              </div>
            </div>
          )}
        </div>
            ...
            {!confirming ? (
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" className="flex-1 h-8 text-xs" disabled={processing} onClick={handleConfirm}>
