...
const STATE_CONFIG: Record<PermissionState, {
  ...
  branch_blocked: {
    icon: GitBranch,
    title: 'Blocked by pending migration',
    description: 'This service cannot start due to a pending database migration (V19__pos_customer_permissions.sql). This is a known issue in the current branch.',
    className: 'permission-banner permission-banner-blocked',
  },
  hidden: {
    icon: ShieldOff,
      ...
      <div className="text-center max-w-md animate-fade-in">
        ...
        {state === 'branch_blocked' && (
          <div className="mt-5 inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-warning/8 border border-warning/15 text-xs text-warning-foreground">
            ...
            Migration V19 — iam-service startup blocked
          </div>
        )}
      </div>
    </div>
