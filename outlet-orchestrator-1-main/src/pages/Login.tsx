...
const AUTH_ERROR_MESSAGES: Record<AuthErrorType, { title: string; message: string; icon: React.ReactNode }> = {
  ...
  branch_blocked: {
    title: 'Service preview — authentication pending',
    message: 'The identity service is currently unavailable in this environment due to a pending backend migration (V19). This does not affect production readiness planning.',
    icon: <ServerCrash className="h-4 w-4" />,
  },
};
...
export default function Login() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (email === 'locked@demo.com') {
        setAuthError('account_locked');
      } else if (email === 'suspended@demo.com') {
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-sm space-y-8">
          ...
          <div className="space-y-2 pt-2">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Demo Shortcuts</p>
            <div className="flex flex-wrap gap-2">
              {[
                { label: 'System Admin', email: 'admin@demo.com' },
                { label: 'Regional Mgr', email: 'regional@demo.com' },
                { label: 'Outlet Mgr', email: 'outlet@demo.com' },
              ].map((d) => (
                <button
                  key={d.email}
                  type="button"
                  ...
                  className="text-xs px-2.5 py-1.5 rounded-md border bg-card text-foreground hover:bg-accent transition-colors"
                >
                  {d.label}
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {[
                { label: 'Locked', email: 'locked@demo.com' },
                { label: 'Suspended', email: 'suspended@demo.com' },
                { label: 'Auth Error', email: 'error@demo.com' },
              ].map((d) => (
                <button
                  key={d.email}
                  type="button"
                  ...
                  className="text-xs px-2.5 py-1.5 rounded-md border border-destructive/20 bg-destructive/5 text-foreground hover:bg-destructive/10 transition-colors"
                >
                  {d.label}
                </button>
              ))}
