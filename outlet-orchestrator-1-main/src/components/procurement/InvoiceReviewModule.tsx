...
export function InvoiceReviewModule() {
  ...
  const handleApprove = (inv: SupplierInvoice) => {
    setInvoices(prev => prev.map(i => i.id === inv.id ? { ...i, status: 'approved' as any, reviewedBy: 'Current User', reviewedAt: new Date().toISOString() } : i));
    setSelected(null);
  ...
  const handleDispute = () => {
    ...
    setInvoices(prev => prev.map(i => i.id === selected.id ? { ...i, status: 'disputed' as any, disputeReason } : i));
