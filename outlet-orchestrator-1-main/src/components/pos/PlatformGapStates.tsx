    ...
    <div className="surface-elevated p-6 space-y-4">
      ...
      <div className="flex items-start gap-2 p-3 rounded-md bg-info/5 border border-info/10">
        ...
        <p className="text-[11px] text-info leading-relaxed">
          This feature is implemented in the backend service source and tested, but requires gateway route registration and permission bootstrap to become available in production.
        </p>
      </div>
    </div>
  );
    ...
    <div className="surface-elevated p-6 space-y-4">
      ...
      <div className="flex items-start gap-2 p-3 rounded-md bg-muted/50">
        ...
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          The IAM service migration <code className="text-[10px] bg-muted px-1 rounded">V19__pos_customer_permissions.sql</code> needs to run successfully before these permissions become available.
        </p>
      </div>
    </div>
  );
...
export function PreviewBlockedBanner({ context }: { context?: string }) {
  return (
    <div className="flex items-start gap-3 p-4 rounded-lg bg-muted/50 border">
      ...
      <div>
        <p className="text-xs font-semibold text-foreground">Preview Environment</p>
        <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">
          {context || 'This feature is available in production but is restricted in the current preview environment.'}
        </p>
      </div>
    </div>
  );
