    ...
    <div className="p-6 space-y-5 animate-fade-in">
      ...
      {showBlocker && (
        <p className="text-[10px] text-muted-foreground text-center italic">
          Displaying mock layout preview — live operations require gateway route activation and IAM permission bootstrap
        </p>
      )}
    </div>
  );
      ...
      {readOnly && (
        <div className="flex items-start gap-2 p-3 rounded-md bg-muted/50 border">
          ...
          <p className="text-[11px] text-muted-foreground">
            View-only mode — table operations require gateway route activation and permission bootstrap.
          </p>
        </div>
      )}
    </div>
