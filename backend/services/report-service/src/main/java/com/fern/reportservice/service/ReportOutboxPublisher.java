    ...
    public void publishPending() {
        ...
        for (JdbcOutboxPublisherSupport.ClaimedOutboxEvent event : events) {
            try {
                ...
                JdbcOutboxPublisherSupport.markPublished(jdbcTemplate, "report.outbox_event", event.id(), clock.instant());
                ...
                log.warn(
                        "report_outbox_publish_failed eventId={} eventType={} retryCount={} terminal={} reason={}",
                        event.id(),
                        event.eventType(),
                        outcome.retryCount(),
                ...
                if (outcome.terminalFailure()) {
                    ...
                    try {
                        operationalAlertPublisher.publish(
                                ...
                                "HIGH",
                                "Report outbox publish failed for " + event.eventType(),
                                null,
                                null,
                                null,
                                event.aggregateType(),
                        ...
                        log.error(
                                "report_outbox_terminal_alert_failed eventId={} eventType={} reason={}",
                                event.id(),
                                event.eventType(),
