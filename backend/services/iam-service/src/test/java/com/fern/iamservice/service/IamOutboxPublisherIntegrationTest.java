...
@SpringBootTest
class IamOutboxPublisherIntegrationTest {
    ...
    static void configure(DynamicPropertyRegistry registry) {
        ...
        registry.add("fern.security.jwt.secret", () -> "iam-outbox-publisher-secret-0123456789012345678901234");
    ...
    private IamOutboxPublisher publisher;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.openMocks(this);
        jdbcTemplate.execute("TRUNCATE TABLE iam.outbox_event");
        publisher = new IamOutboxPublisher(
    ...
    @Test
    void shouldReclaimStaleInProgressIamOutboxEvent() {
        ...
        verify(kafkaTemplate).send(eq("iam.user.changed"), eq("77"), anyString());
    ...
    @Test
    void shouldNotReclaimFreshInProgressIamOutboxEvent() {
    ...
    @Test
    void shouldNotReclaimTerminalFailedIamOutboxEvent() {
    ...
    private String status(UUID eventId) {
        return jdbcTemplate.queryForObject("SELECT status FROM iam.outbox_event WHERE id = ?", String.class, eventId);
    }

    private OffsetDateTime publishedAt(UUID eventId) {
        return jdbcTemplate.query(
                "SELECT published_at FROM iam.outbox_event WHERE id = ?",
    ...
    private String lastError(UUID eventId) {
        return jdbcTemplate.query(
                "SELECT last_error FROM iam.outbox_event WHERE id = ?",
