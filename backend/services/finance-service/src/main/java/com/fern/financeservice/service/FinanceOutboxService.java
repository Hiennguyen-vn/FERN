...
public class FinanceOutboxService {
    ...
    public void emitPayrollCalculated(PayrollPeriodRecord period, PayrollRunResponse run, FernPrincipal principal, String correlationId) {
        ...
        PayrollCalculatedEvent event = new PayrollCalculatedEvent(
                ...
                "finance-service",
                correlationId,
                payrollCalculatedIdempotencyKey(run.id()),
                run.id(),
                run.payrollPeriodId(),
                period.regionId(),
                run.runDate(),
    ...
    public void emitPayrollPosted(
        ...
        PayrollPostedEvent event = new PayrollPostedEvent(
                ...
                "finance-service",
                correlationId,
                payrollPostedIdempotencyKey(run.id()),
                run.id(),
                run.payrollPeriodId(),
                period.regionId(),
                run.runDate(),
    ...
    public void emitExpensePosted(
        ...
        ExpensePostedEvent event = new ExpensePostedEvent(
                ...
                "finance-service",
                correlationId,
                expensePostedIdempotencyKey(expenseRecordId),
                expenseRecordId,
                regionId,
                outletId,
                employeeId,
    ...
    private String payrollCalculatedIdempotencyKey(Long runId) {
        return "payroll.calculated:run:" + runId;
    }

    private String payrollPostedIdempotencyKey(Long runId) {
        return "payroll.posted:run:" + runId;
    }

    private String expensePostedIdempotencyKey(Long expenseRecordId) {
        return "finance.expense.posted:expense:" + expenseRecordId;
    }
