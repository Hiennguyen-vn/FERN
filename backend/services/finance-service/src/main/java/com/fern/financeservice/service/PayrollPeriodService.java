...
public class PayrollPeriodService {
    ...
    public PayrollPeriodResponse createPayrollPeriod(FernPrincipal principal, CreatePayrollPeriodRequest request, String correlationId) {
        ...
        validatePayrollPeriodDates(request);
    ...
    private void validatePayrollPeriodDates(CreatePayrollPeriodRequest request) {
        if (request.endDate().isBefore(request.startDate())) {
            throw new BadRequestException("Payroll period endDate must be on or after startDate");
        }
        if (request.payDate() != null && request.payDate().isBefore(request.endDate())) {
            throw new BadRequestException("Payroll period payDate must be on or after endDate");
        }
    }
