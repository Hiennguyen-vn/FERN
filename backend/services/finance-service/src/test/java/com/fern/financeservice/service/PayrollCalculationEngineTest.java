...
class PayrollCalculationEngineTest {
    ...
    void shouldReturnZeroPayWhenAttendanceIsEmpty() throws Exception {
        ...
        assertThat(computation.primaryContractId()).isEqualTo(701L);
        assertThat(computation.primaryOutletId()).isNull();
        assertThat(computation.grossPay()).isEqualByComparingTo("0.00");
        assertThat(computation.deductionAmount()).isEqualByComparingTo("0.00");
        assertThat(computation.taxAmount()).isEqualByComparingTo("0.00");
        assertThat(computation.netPay()).isEqualByComparingTo("0.00");
        assertThat(computation.workDays()).isEqualByComparingTo("0.00");
        assertThat(computation.workHours()).isEqualByComparingTo("0.00");
        assertThat(computation.overtimeHours()).isEqualByComparingTo("0.00");
        assertThat(computation.allocations()).isEmpty();
    }

    @Test
    void shouldPreserveDecimalPrecisionFromPolicyValues() throws Exception {
        PayrollPeriodRecord period = new PayrollPeriodRecord(1L, 10L, LocalDate.of(2026, 3, 10), LocalDate.of(2026, 3, 10), LocalDate.of(2026, 3, 12), "DRAFT");
        EffectiveContract contract = new EffectiveContract(702L, 502L, 10L, "PART_TIME", "HOURLY", new BigDecimal("0.10"), "TAX-002", LocalDate.of(2026, 3, 1), null);
        List<ApprovedAttendance> attendance = List.of(
                new ApprovedAttendance(13L, 23L, 502L, 10L, 401L, 702L, LocalDate.of(2026, 3, 10), "LATE", new BigDecimal("1.0"), new BigDecimal("1.0"))
        );
        ...
        PayrollEmployeeComputation computation = payrollCalculationEngine.computeEmployeePayroll(
                ...
                2,
                json("{\"defaultMultiplier\":\"1.5\"}"),
                json("{\"mealPerWorkDay\":\"0.10\",\"transportPerWorkDay\":\"0.20\"}"),
                json("{\"latePenaltyPerCount\":\"0.05\"}"),
                json("{\"rate\":\"0.10\"}")
        );

        assertThat(computation.grossPay()).isEqualByComparingTo("0.55");
        assertThat(computation.deductionAmount()).isEqualByComparingTo("0.05");
        assertThat(computation.taxAmount()).isEqualByComparingTo("0.05");
