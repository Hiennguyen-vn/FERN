...
@SpringBootTest
class HrOutboxPublisherIntegrationTest {
    ...
    static void configure(DynamicPropertyRegistry registry) {
        ...
        registry.add("fern.security.jwt.secret", () -> "hr-outbox-publisher-secret-012345678901234567890123456789");
    }
    ...
    private HrOutboxPublisher publisher;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.openMocks(this);
        jdbcTemplate.execute("TRUNCATE TABLE hr.outbox_event");
        publisher = new HrOutboxPublisher(
    ...
    @Test
    void shouldNotReclaimFreshInProgressHrOutboxEvent() {
    ...
    @Test
    void shouldNotReclaimTerminalFailedHrOutboxEvent() {
    ...
    private UUID insertOutbox(String status, int retryCount, Instant lastAttemptAt) {
        ...
        jdbcTemplate.update("""
                ...
                ) VALUES (?, 'ATTENDANCE_EVENT', '501', 'hr.attendance.recorded', '101', CAST(? AS jsonb), ?, ?, ?, ?, ?)
                """,
                eventId,
                "{\"id\":501}",
                status,
    ...
    private String status(UUID eventId) {
        return jdbcTemplate.queryForObject("SELECT status FROM hr.outbox_event WHERE id = ?", String.class, eventId);
    }

    private OffsetDateTime publishedAt(UUID eventId) {
        return jdbcTemplate.query(
                "SELECT published_at FROM hr.outbox_event WHERE id = ?",
    ...
    private String lastError(UUID eventId) {
        return jdbcTemplate.query(
                "SELECT last_error FROM hr.outbox_event WHERE id = ?",
