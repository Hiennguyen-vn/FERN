...
public class HrBeans {
    ...
    @Value("${fern.clients.org.connect-timeout-ms:5000}")
    private int orgConnectTimeoutMs;

    @Value("${fern.clients.org.read-timeout-ms:10000}")
    private int orgReadTimeoutMs;

    @Value("${fern.clients.org.circuit-breaker.failure-rate-threshold:50}")
    private float orgFailureRateThreshold;

    @Value("${fern.clients.org.circuit-breaker.sliding-window-size:10}")
    private int orgSlidingWindowSize;

    @Value("${fern.clients.org.circuit-breaker.minimum-number-of-calls:5}")
    private int orgMinimumNumberOfCalls;

    @Value("${fern.clients.org.circuit-breaker.wait-duration-open-seconds:30}")
    private long orgWaitDurationOpenSeconds;
    ...
    OperationalAlertPublisher operationalAlertPublisher(
        ...
        return new KafkaOperationalAlertPublisher(kafkaTemplate, objectMapper, clock, "hr-service");
    }
    ...
    RestClient orgRestClient(@Value("${fern.clients.org.base-url:http://localhost:8082}") String baseUrl) {
        ...
        requestFactory.setConnectTimeout(orgConnectTimeoutMs);
        requestFactory.setReadTimeout(orgReadTimeoutMs);
    ...
    CircuitBreaker orgCircuitBreaker() {
        return CircuitBreaker.of("hr-org-client", CircuitBreakerConfig.custom()
                .failureRateThreshold(orgFailureRateThreshold)
                .slidingWindowSize(orgSlidingWindowSize)
                .minimumNumberOfCalls(orgMinimumNumberOfCalls)
                .waitDurationInOpenState(Duration.ofSeconds(orgWaitDurationOpenSeconds))
                .build());
    }
