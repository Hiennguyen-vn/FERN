...
class InventoryOutboxPublisherIntegrationTest {
    ...
    static void configure(DynamicPropertyRegistry registry) {
        ...
        registry.add("fern.security.jwt.secret", () -> "inventory-outbox-publisher-secret-0123456789012345678901");
    ...
    @Test
    void shouldReclaimStaleInProgressInventoryOutboxEvent() {
        ...
        verify(kafkaTemplate).send("inventory.adjustment.posted", "101", "{\"id\": 8801}");
    ...
    @Test
    void shouldNotReclaimFreshInProgressInventoryOutboxEvent() {
        UUID eventId = insertOutbox("IN_PROGRESS", 1, NOW.minusSeconds(30));

        publisher.publishPending();

        verify(kafkaTemplate, never()).send("inventory.adjustment.posted", "101", "{\"id\":8801}");
    ...
    private UUID insertOutbox(String status, int retryCount, Instant lastAttemptAt) {
        ...
        jdbcTemplate.update("""
                INSERT INTO inventory.outbox_event (
                    ...
                    ?, 'STOCK_ADJUSTMENT', '8801', 'inventory.adjustment.posted', '101', CAST(? AS jsonb), ?,
                    ?, ?, ?, ?
                )
                """,
                eventId,
