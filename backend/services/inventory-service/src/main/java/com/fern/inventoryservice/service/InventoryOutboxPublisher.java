    ...
    public void publishPending() {
        ...
        for (JdbcOutboxPublisherSupport.ClaimedOutboxEvent event : events) {
            try {
                ...
                log.warn(
                        "inventory_outbox_publish_failed eventId={} eventType={} retryCount={} terminal={} reason={}",
                        event.id(),
                        event.eventType(),
                        outcome.retryCount(),
                ...
                if (outcome.terminalFailure()) {
                    ...
                    try {
                        operationalAlertPublisher.publish(
                                ...
                                "HIGH",
                                "Inventory outbox publish failed for " + event.eventType(),
                                null,
                                null,
                                null,
                                event.aggregateType(),
                        ...
                        log.error(
                                "inventory_outbox_terminal_alert_failed eventId={} eventType={} reason={}",
                                event.id(),
                                event.eventType(),
