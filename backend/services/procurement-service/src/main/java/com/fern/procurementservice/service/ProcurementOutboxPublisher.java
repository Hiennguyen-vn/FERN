...
public class ProcurementOutboxPublisher {
    ...
    public ProcurementOutboxPublisher(
        ...
        this.terminalFailureCounter = Counter.builder("fern_outbox_terminal_failures_total")
                .tag("service", "procurement-service")
    ...
    public void publishPending() {
        ...
        for (JdbcOutboxPublisherSupport.ClaimedOutboxEvent event : events) {
            try {
                ...
                JdbcOutboxPublisherSupport.markPublished(jdbcTemplate, "procurement.outbox_event", event.id(), clock.instant());
                ...
                log.warn(
                        "procurement_outbox_publish_failed eventId={} eventType={} retryCount={} terminal={} reason={}",
                        event.id(),
                        event.eventType(),
                        outcome.retryCount(),
                ...
                if (outcome.terminalFailure()) {
                    ...
                    try {
                        operationalAlertPublisher.publish(
                                ...
                                "HIGH",
                                "Procurement outbox publish failed for " + event.eventType(),
                                null,
                                null,
                                null,
                                event.aggregateType(),
                        ...
                        log.error(
                                "procurement_outbox_terminal_alert_failed eventId={} eventType={} reason={}",
                                event.id(),
                                event.eventType(),
