    ...
    public void publishPending() {
        for (JdbcOutboxPublisherSupport.ClaimedOutboxEvent event : JdbcOutboxPublisherSupport.claimBatch(
            ...
            try {
                ...
                JdbcOutboxPublisherSupport.markPublished(jdbcTemplate, "org.outbox_event", event.id(), clock.instant());
                ...
                log.warn(
                        "org_outbox_publish_failed eventId={} eventType={} retryCount={} terminal={} reason={}",
                        event.id(),
                        event.eventType(),
                        outcome.retryCount(),
                ...
                if (outcome.terminalFailure()) {
                    ...
                    try {
                        operationalAlertPublisher.publish(
                                ...
                                "HIGH",
                                "Org outbox publish failed for " + event.eventType(),
                                null,
                                null,
                                null,
                                event.aggregateType(),
                        ...
                        log.error(
                                "org_outbox_terminal_alert_failed eventId={} eventType={} reason={}",
                                event.id(),
                                event.eventType(),
