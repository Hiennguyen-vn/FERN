...
public class OrgFinanceClient {
    ...
    public OrgFinanceClient(
            @Qualifier("financeRestClient") RestClient restClient,
            @Qualifier("financeCircuitBreaker") CircuitBreaker circuitBreaker,
            @Qualifier("financeClientSpec") FernDownstreamClientSpec clientSpec,
    ...
    public OutletCloseCheck getOutletCloseCheck(Long outletId, FernPrincipal actor) {
        OutletCloseCheck response = downstreamClientFactory.execute(operation("outlet-close-check"), circuitBreaker, () -> restClient.get()
                .uri(uriBuilder -> uriBuilder
                        .path("/internal/finance/outlet-close-check")
            ...
            if (response == null) {
                throw new DownstreamUnavailableException(
                        "finance-service returned an empty outlet close check response for outlet " + outletId
                );
            }
            return response;
    }
