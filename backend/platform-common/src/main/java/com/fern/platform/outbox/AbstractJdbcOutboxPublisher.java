...
public abstract class AbstractJdbcOutboxPublisher {
    ...
    /**
     * Purge outbox events that have been successfully published and are older than the given retention period.
     * Subclasses should call this from a {@code @Scheduled} method (e.g., daily at 3 AM).
     *
     * @param retention how long to keep published events before purging
    ...
    protected final int purgeSentEvents(Duration retention) {
        ...
        int deleted = jdbcTemplate.update(
                "DELETE FROM " + qualifiedOutboxTable() + " WHERE status = 'PUBLISHED' AND created_at < :cutoff",
